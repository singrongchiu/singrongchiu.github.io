...
- At any given time, exactly 2 plants are Dry (need water), and 1 plant is Watered (does not need water).
...
- If the nozzle is dropped onto a Dry plant, that plant’s state changes from Dry → Watered and counts as 1 watering.
- If the nozzle is dropped onto a Watered plant, nothing happens (no watering counted).

Win Condition:
- The mini-game is completed when the player has successfully watered Dry plants a total of 2 times (i.e., 2 Dry → Watered actions).